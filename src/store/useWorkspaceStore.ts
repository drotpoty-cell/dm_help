...
export const useWorkspaceStore = create<WorkspaceState>((set, get) => ({
  ...
  // ИНИЦИАЛИЗАЦИЯ БАЗЫ ДАННЫХ (включая новые категории)
  heroes: {}, npcs: {}, quests: {}, locations: {}, secrets: {}, loot: {}, events: {},
  characters: {}, extras: {}, bestiary: {}, factions: {}, // <--- НОВЫЕ СУЩНОСТИ ДОБАВЛЕНЫ СЮДА
  ...
  setLibrary: (library: any) => {
    ...
    set({
      ...
      // Подхватываем новые данные из импорта шаблона
  ...
  advanceTime: (hoursToAdd: number) => set((state: any) => {
    ...
    // ОБРАБАТЫВАЕМ РАСПИСАНИЯ ДЛЯ ВСЕХ ТИПОВ ПЕРСОНАЖЕЙ (старых и новых)
  ...
  updateEntity: (category: string, id: string, data: any) => set((state: any) => {
    const result = updateInLibrary(state, category as any, id, (prev) => {
      ...
      // Поддерживаем defaultLocationId для расписаний новых категорий
  ...
  deleteEntity: (category: string, id: string) => set((state: any) => {
    ...
    if (category === 'locations') {
      ...
      for (const nodeId of removedNodeIds) {
        // Универсальная очистка ссылок для всех сущностей
  ...
  updateQuestStatus: (questId: string, newStatus: string) => set((state: any) => {
    ...
    // Если квест провален/завершен, вешаем флаг needsUpdate на локацию и её ключевых лиц
