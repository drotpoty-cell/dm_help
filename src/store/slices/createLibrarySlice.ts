...
import { migrateLegacyLibrary, type LegacySnapshot } from '@/utils/migrateLegacyLibrary'
...
export interface LibrarySlice {
  ...
  clearNeedsUpdate: (type: 'node' | 'character' | 'enemy', targetId: string) => void
  /**
   * Переносит данные из устаревших ключей npcs/crowd (кампании, сохранённые до слияния
   * категорий) в characters/extras. Вызывается ТОЛЬКО явным действием ГМа из UI
   * (кнопка «Мигрировать данные кампании» с подтверждением) — никогда автоматически,
   * чтобы не рисковать перезаписью облачной копии автосохранением до проверки результата.
   */
  migrateLegacyData: () => { migratedNpcCount: number; migratedCrowdCount: number }
}
...
export const createLibrarySlice: StateCreator<WorkspaceState, [], [], LibrarySlice> = (set) => ({
  ...
  updateEntity: (category, id, data) =>
    set((state) => {
      const result = updateInLibrary(state, category, id, (prev) => {
        ...
        if (
          (category === 'characters' || category === 'extras') &&
          data &&
          typeof data === 'object' &&
          'locationId' in data
        ) {
  ...
  importAIData: (data) =>
    set((state) => {
      ...
      return {
        ...
        // Данные от ИИ по-прежнему могут приходить под старыми ключами npcs/crowd
        // (шаблон генерации ещё используют другие клиенты) — сразу кладём их в
        // канонические characters/extras, при этом npc-шные генерации помечаем как важные.
        characters: mergeData(
          mergeData(state.characters, data.characters),
          (data.npcs || []).map((npc: any) => ({ ...npc, isImportant: true }))
        ),
        enemies: mergeData(state.enemies, data.enemies),
        // ВАЖНО: "extras" в ответе ИИ — это второстепенные персонажи массовки (бывший "crowd"),
        // а не интерактивные объекты тактической карты — те приходят под ключом "interactive".
        // Старые ответы ИИ/промпты могли всё ещё присылать их под ключом "crowd" — принимаем оба.
        extras: mergeData(mergeData(state.extras, data.extras), data.crowd),
        interactive: mergeData(state.interactive, data.interactive),
  ...
  migrateLegacyData: () => {
    let result = { migratedNpcCount: 0, migratedCrowdCount: 0 }
    set((state) => {
      const migration = migrateLegacyLibrary(state as unknown as LegacySnapshot)
      result = {
        migratedNpcCount: migration.migratedNpcCount,
        migratedCrowdCount: migration.migratedCrowdCount,
      }
      if (migration.migratedNpcCount === 0 && migration.migratedCrowdCount === 0) return state
      return {
        ...state,
        characters: migration.characters,
        extras: migration.extras,
        // Старые ключи больше не нужны — обнуляем, чтобы следующий снапшот
        // автосохранения их не тащил с собой.
        npcs: undefined,
        crowd: undefined,
