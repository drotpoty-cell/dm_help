...
export interface SystemSlice {
  ...
  /** Атмосфера и правила генерации мира (Блок 6, корректировка) — кастомный системный
   *  промпт КОНКРЕТНОЙ кампании. В отличие от useSettingsStore.systemPrompt (личный тон
   *  ГМа в браузере, не путешествует между устройствами), это свойство мира — сохраняется
   *  в JSONB-снапшоте кампании в Supabase вместе с остальными данными и одинаково для
   *  любого устройства, с которого открыта эта кампания. Пусто по умолчанию — генератор
   *  без этого поля выдаёт нейтральный текст в духе базового D&D 5e, никакого жёстко
   *  зашитого сеттинга или жанра в коде нет.
   */
  ...
  setWorldSystemPrompt: (prompt: string) => void
}
...
export const createSystemSlice: StateCreator<WorkspaceState, [], [], SystemSlice> = (set, get) => ({
  ...
  setWorldSystemPrompt: (prompt) => set({ worldSystemPrompt: prompt }),
