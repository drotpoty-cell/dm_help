...
import { buildSmartAiContext } from '@/utils/aiTemplateGenerator';
...
interface AiWandProps {
  ...
  title?: string;
  // Режим генерации — определяет и тон промпта, и то, какие поля contextData реально
  // уходят в запрос (см. buildSmartAiContext, Блок 6, п.2 — "умный контекст").
  mode?: 'character' | 'location' | 'loot' | 'quest' | 'general';
}

export const AiWand: React.FC<AiWandProps> = ({ currentValue, contextData, onApply, title, mode = 'general' }) => {
  ...
  const handleGenerate = async (useCurrentValue: boolean) => {
    ...
    try {
      if (useCurrentValue) {
        ...
        // "Умный контекст" (Блок 6, п.2): режим генерации определяет, какие поля из
        // contextData реально уходят в промпт — например, реплика NPC не тащит с собой
        // данные локации/расписание/ассортимент торговца, которые ей не нужны.
        const scopedContext = buildSmartAiContext(mode, contextData);
        const response = await generateAiText(finalPrompt, JSON.stringify(scopedContext));
