...
// --- РЕДАКТОР ГЛАВЫ (Tiptap + @mention, Блок 4, п.5) ---
    ...
    <div className="flex-1 flex flex-col mt-4 min-h-[500px]">
      <div className="flex justify-between items-end mb-2">
        <span className="text-[10px] text-zinc-500 font-medium">
          Наберите @, чтобы упомянуть персонажа, локацию или квест — появится подсказка
        </span>
        <AiWand
          mode="general"
...
export default function StoryBoard() {
  ...
  const addPart = () => setStory([...story, { id: generateId('part'), title: 'Новая Часть', chapters: [] }])
      ...
      <div className="w-80 border-r border-zinc-900 bg-zinc-950 flex flex-col shrink-0 z-20 shadow-2xl">
        <div className="p-5 border-b border-zinc-900 bg-zinc-900/40">
          ...
          <button onClick={() => {
            ...
            if (title) addPlotNode({ id: generateId('plot'), title, description: '', status: 'active' });
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-8 custom-scrollbar">
          {story.length === 0 && <div className="text-zinc-600 text-[10px] text-center mt-10 uppercase tracking-widest font-bold border-2 border-dashed border-zinc-800 p-6 rounded-xl">Нажмите &ldquo;+ Часть&rdquo;, чтобы начать строить историю</div>}
              ...
              <div className="w-72 pl-10 border-l border-zinc-800/50 hidden lg:flex flex-col gap-10 shrink-0">
                <div>
                  ...
                  <ul className="text-xs text-zinc-400 space-y-4 leading-relaxed">
                    ...
                    <li className="flex gap-3"><span className="text-emerald-500/50 mt-0.5">•</span> Используйте &ldquo;Правило 3 улик&rdquo;: любая важная инфа должна упоминаться трижды.</li>
