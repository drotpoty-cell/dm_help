    ...
    <div className="flex flex-col gap-4">
      ...
      <div>
        <div className="flex justify-between items-center mb-2">
          ...
          <AiWand 
            mode="location"
            currentValue={location.detailedDescription || ''}
            contextData={location}
            onApply={(text) => onUpdate({ detailedDescription: text })}
          />
        </div>
        <RichTextEditor
          content={location.detailedDescription || ''}
          onChange={(html) => onUpdate({ detailedDescription: html })}
          placeholder="Подробности, атмосфера, кто здесь бывает... Наберите @, чтобы сослаться на NPC или предмет"
          minHeight="140px"
        />
      </div>
      ...
      <div className="flex gap-3 pt-4 border-t border-zinc-800/50">
        <button
          onClick={() => {
            const newChecks = [...checks, { id: generateId('chk'), skill: 'Внимательность', dc: 15, passText: '', failText: '' }];
            onUpdate({ checks: newChecks });
          }}
