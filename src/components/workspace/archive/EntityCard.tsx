...
import { Hero, Character, Quest, Loot } from '@/types/workspace'
...
export default function EntityCard({ entity, type, isActive, onClick }: EntityCardProps) {
  ...
  const isMerchant = type === 'characters' && entity.occupation?.toLowerCase().includes('торговец');
  ...
  const renderBadge = () => {
    ...
    if (type === 'characters' && entity.isImportant) {
      return (
        <span className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded border bg-amber-500/10 text-amber-400 border-amber-500/30">
          Важный
        </span>
      );
    }
  ...
  return (
    ...
    >
      ...
      {isMerchant && (entity as Character).personalLoot && (
        <div className="mt-2 pt-2 border-t border-zinc-800/50">
          ...
          <div className="text-[10px] text-orange-400/80 font-medium truncate">
            {(entity as Character).personalLoot}
          </div>
        </div>
      )}

      {/* Индикация типа для персонажей */}
      {type === 'characters' && (entity as Character).occupation && !isMerchant && (
        <div className="text-[9px] text-zinc-600 font-bold uppercase tracking-tighter">
          {(entity as Character).occupation}
        </div>
      )}
    </div>
  )
