...
export const DataSyncModal = ({ isOpen, onClose }: DataSyncModalProps) => {
  ...
  const { plotNodes, characters, extras, locations, importAIData, worldSystemPrompt } = useWorkspaceStore()
  ...
  const exportEmptyTemplate = () => {
    const template = generateAIPromptTemplate(undefined, worldSystemPrompt)
    handleDownload('world-template.txt', template)
  }

  const exportCurrentWorld = () => {
    const data = { plotNodes, characters, extras, locations }
    const template = generateAIPromptTemplate(data, worldSystemPrompt)
  ...
  const handleImport = () => {
    try {
      ...
      toast.error('Ошибка при парсинге JSON')
    }
  }
