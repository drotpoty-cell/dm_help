...
export const InitiativeTracker = () => {
  const { combat, startCombat, endCombat, nextTurn, updateCombatantInitiative, activeLocalMapId, heroes, characters, enemies, extras } = useWorkspaceStore()
    ...
    <div className="p-4 border rounded bg-gray-800 text-white">
      ...
      <div className="space-y-2">
        {combat.participants.map((p, index) => {
          const entityByType: Record<string, any> = { hero: heroes, npc: characters, enemies: enemies, extra: extras };
          const entity = entityByType[p.type]?.[p.entityId] || { hp: p.hp, maxHp: p.maxHp, ac: p.ac };
          return (
            <div 
              key={p.tokenId} 
