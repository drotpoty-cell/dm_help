...
// (вынесено в utils/createBlankEntity.ts, чтобы ArchiveBoard мог переиспользовать)

export default function CockpitSidebar() {
  const [isSettingsOpen, setIsSettingsOpen] = useState(false)
  const [isDataSyncOpen, setIsDataSyncOpen] = useState(false)
  const [activeTab, setActiveTab] = useState<LibraryCategory>('characters')
  ...
  const handleCreate = () => {
    const entity = createBlankEntity(activeTab)
    addEntity(activeTab, entity)
  ...
  return (
    <aside className="w-72 shrink-0 flex flex-col h-full bg-zinc-950 border-r border-zinc-900">
      <div className="p-3 border-b border-zinc-900 space-y-3 shrink-0">
        <div className="flex items-center justify-between px-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              title={`Добавить: ${INVENTORY_TABS.find((t) => t.id === activeTab)?.label}`}
              onClick={handleCreate}
              className="w-5 h-5 flex items-center justify-center rounded bg-indigo-600/20 text-indigo-400 border border-indigo-500/30 hover:bg-indigo-600/40 transition-colors"
            >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar p-3">
        {filteredItems.length === 0 ? (
          <p className="text-[10px] text-zinc-600 italic tracking-wider text-center py-8">
            {query.trim() ? 'Ничего не найдено' : 'Список пуст. Нажмите "+", чтобы добавить.'}
          </p>
        ) : (
      ...
      {(activeView === 'map' || activeView === 'dashboard' || activeView === 'table') && activeLocalMapId && (
      ...
      <div className="p-3 border-t border-zinc-900 shrink-0 flex flex-col gap-1">
        <button
          type="button"
