...
const SmartEditor = ({ value, onChange, placeholder, mode, contextData }: { value: string, onChange: (val: string) => void, placeholder: string, mode?: 'location' | 'general', contextData?: any }) => {
  ...
  // ВАЖНО: раньше здесь текст вставлялся через dangerouslySetInnerHTML, что открывало
  // XSS — любой сохранённый текст (например, скопированный в описание локации/NPC)
  // с тегами вроде <img onerror=...> выполнился бы в браузере ГМа. Теперь текст всегда
  // рендерится как обычные React-узлы (авто-экранирование), а подсветкой "@Имя"
  // помечаются только реальные совпадения по известным сущностям.
  const renderFormattedText = (text: string) => {
    ...
    const names = Array.from(
      new Set(
        allEntities
          .map((entity: any) => entity.name || entity.title)
          .filter((name): name is string => Boolean(name))
      )
    ).sort((a, b) => b.length - a.length) // длинные имена в приоритете, чтобы избежать частичных совпадений
    ...
    const pattern = new RegExp(`@(${names.map(escapeRegExp).join('|')})(?![\\wа-яА-ЯёЁ])`, 'gi')
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      parts.push(
        <span key={key++} className="text-indigo-400 bg-indigo-900/30 px-1.5 py-0.5 rounded font-bold cursor-pointer hover:bg-indigo-900/50 border border-indigo-900/50">
          @{match[1]}
        </span>
      )
...
export default function Sidebar({ selectedNodeId, onClose }: SidebarProps) {
  ...
  const attachNpc = (e: React.ChangeEvent<HTMLSelectElement>) => {
    if (e.target.value) updateEntity('characters', e.target.value, { locationId: selectedNodeId })
  }
